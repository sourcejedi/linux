...
void wait_lapic_expire(struct kvm_vcpu *vcpu)
{
	...
	if (guest_tsc < tsc_deadline) {
		...
		/*
		 * Safety check to avoid locking up host cpu.  Instead, the
		 * emulated deadline timer will fire early (just as if we
		 * hadn't implemented lapic_timer_advance_ns).  Note firing
		 * early is is incorrect even if only by a few cycles.  The
		 * textbook definition of a delay / timer is that you can not
		 * wake earlier, only later.
		 *
		 * Assumption: TSC "catchup" to the nominal frequency has
		...
		if (delay > max_delay) {
			WARN_ONCE(1,
			          "%llu cycles is too long to busy-wait.\n",
				  delay);
			delay = max_delay;
		}
...
/*
 * kvm_apic_tsc_update - called on update or frequency change of guest tsc
 *
